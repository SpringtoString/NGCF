...
class NGCF(nn.Module):
    ...
    def __init__(self, n_user, n_item, norm_adj, gcn_layers=2, drop_rate=0,
        ...
        ''' GCN 相关参数'''
        self.GCN_W1 = nn.ModuleList([nn.Linear(embedding_size,embedding_size) for _ in range(gcn_layers)])
        self.GCN_W2 = nn.ModuleList([nn.Linear(embedding_size, embedding_size) for _ in range(gcn_layers)])
        for i in range(gcn_layers):
            nn.init.normal_(self.GCN_W1[i].weight, mean=0, std=init_std)
            nn.init.normal_(self.GCN_W2[i].weight, mean=0, std=init_std)

        self.sparse_norm_adj = self._convert_sp_mat_to_sp_tensor(norm_adj).to(device)
        self.to(device)
    ...
    def forward(self, input_dict):
        ...
        '''
        ''' GCN '''
        user_emb_matrix,item_emb_matrix = self.GCN()

        ''' get final embedding vector '''
        users, pos_items, neg_items = input_dict['users'], input_dict['pos_items'], input_dict['neg_items']

        user_vector =  F.embedding(users, user_emb_matrix)
        pos_items_vector = F.embedding(pos_items, item_emb_matrix)
        neg_items_vector = F.embedding(neg_items, item_emb_matrix)
    ...
    def GCN(self):
        ego_embedding = torch.cat([self.embedding_dict['user_emb'].weight, self.embedding_dict['item_emb'].weight],
                                  dim=0)
        all_embeddings = [ego_embedding]
        for i in range(self.gcn_layers):
            T = torch.sparse.mm(self.sparse_norm_adj, all_embeddings[i])
    ...
    def rating(self, user_batch, all_item):
        user_emb_matrix, item_emb_matrix = self.GCN()
        user_vector = F.embedding(user_batch, user_emb_matrix)
        pos_items_vector = F.embedding(all_item, item_emb_matrix)
